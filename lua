...
local window = library:AddWindow("ลุงหลาม 49 | เรื่องจริงครับพี่ " .. displayName, {
    main_color = Color3.fromRGB(255, 215, 0), -- เปลี่ยนเป็นโทนสีส้ม (Orange)
...
local function updateAdvancedStats(targetPlayer)
    if not targetPlayer then
        enemyHealthLabel.Text = "เลือดศัตรู: ยังไม่ได้ทำ"
        playerDamageLabel.Text = "ดาเมจเรา: ยังไม่ได้ทำ"
        hitsToKillLabel.Text = "จำนวนหมัด: ยังไม่ได้ทำ"
        return
    end
...
local Calculadora = window:AddTab("คำนวณเลือดดาเมจ", Color3.fromRGB(255, 215, 0)
